...
def generate_rasters(unit, cherries, TrialInfo, sentence, args):
    
    # GET INDICES
    IXs_to_target_trials, sentences, labels = get_indices_to_target_trials(TrialInfo, sentence, args)
...
def get_indices_to_target_trials(TrialInfo, sentence, args):
    '''
    ...
    if args.block == 'syntactic':
        ...
        if args.patient not in [83, 84, 86, 88, 89]:
            sentence_number_control = list(set([TrialInfo['ref2target'][IX] for IX in IXs_sentences[-1]]))
            assert len(sentence_number_control) == 1
            control_sentence = []
            if sentence_number_control[0]>0:
                IXs_control = [IX for IX, sent_num in enumerate(TrialInfo['sentence_numbers']) if sent_num == sentence_number_control[0]]
...
def spiketrain2raster(spike_trains):
    ...
    for i_trial, spike_train in enumerate(spike_trains):
        IX_spikes = [int(t) for t in spike_train[0] if t>=0] # remove negative spike times (before fixation), since raster_matrix starts at t=0
